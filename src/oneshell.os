...
Function ValueIsFilled(Value)
	Return ЗначениеЗаполнено(Value)
EndFunction
...
Procedure Init()
	...
	InitialTokensOfExpression.Add("Not");
...
Function ScanComment(Scanner)
	...
	Len = 0;
	Char = NextChar(Scanner);
	While Char <> Chars.ПС And Char <> "" Do
		Len = Len + 1;
		Char = NextChar(Scanner);
	EndDo;
...
Function ScanStringLen(Scanner)
	...
	Len = 1;
	Char = NextChar(Scanner);
	While Char <> """" And Char <> Chars.ПС And Char <> "" Do
		Len = Len + 1;
		Char = NextChar(Scanner);
	EndDo;
	If Char = Chars.ПС Then
	...
	If BasicLiterals.Find(Tok) <> Undefined Then
		If Tok = Tokens.String Then
			...
			While Next(Parser) = Tokens.String Do
				StrList.Add(Parser.Val);
			EndDo;
			Operand = BasicLitExpr(Tok, StrConcat(StrList, Chars.ПС));
		Else
...
#EndRegion // Backends
	...
	Если FormOutput = "Lexems" Тогда
		...
		While Scan(Scanner) <> Eof Do
			Сообщить(StrTemplate("%1: %2 -- `%3`", Scanner.Line, Scanner.Tok, Scanner.Lit));
		EndDo;
